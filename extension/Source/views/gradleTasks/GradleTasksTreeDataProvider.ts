	...
	private async buildTreeItems(): Promise<vscode.TreeItem[]> {
		...
		Array.from(pinnedTasks.keys()).forEach((taskId: TaskId) => {
			...
			if (taskArgs) {
				Array.from(taskArgs.values()).forEach((args: TaskArgs) => {
					...
					const gradleTaskTreeItem = buildPinnedTaskTreeItem(
						pinnedTasksItem,
						pinnedTask,
						this.icons,
					);
		...
		if (pinnedTasksMap.size) {
			if ((await this.rootProjectStore.getProjectRoots()).length === 1) {
				pinnedTasksItem.setChildren(
					pinnedTasksMap.values().next().value,
				);
			} else {
				...
				pinnedTasksMap.forEach((value, key) => {
					...
					if (rootProject) {
						const pinnedTasksRootProjectTreeItem =
							new PinnedTasksRootProjectTreeItem(
								path.basename(key),
	...
	public static buildItemsTreeFromTasks(
		...
		tasks.forEach((task) => {
			...
			if (isWorkspaceFolder(task.scope) && isGradleTask(task)) {
				const rootProject = rootProjectStore.get(
					definition.projectFolder,
				);
				if (!rootProject) {
					return;
				...
				let projectTreeItem = projectTreeItemMap.get(
					projectPath.join(":"),
				);
				if (!projectTreeItem) {
				...
				let parentTreeItem: ProjectTreeItem | GroupTreeItem =
					projectTreeItem;
