...
describe(getSuiteName("Extension"), () => {
	...
	describe("Task provider", () => {
		...
		it("should run a gradle task", async () => {
			const task = (
				await vscode.tasks.fetchTasks({ type: "gradle" })
			).find(({ name }) => name === "hello");
			assert.ok(task);
			...
			assert.ok(
				loggerAppendLineSpy.calledWith(
					sinon.match("Completed build: hello")
				)
			);
		});
