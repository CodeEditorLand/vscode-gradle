	...
	describe("Without a multi-root workspace", () => {
		describe("With no gradle tasks", () => {
			...
			it('should build a "No Tasks" tree item when no tasks are found', async () => {
				...
				assert.strictEqual(
					noTasksTreeItem.command.command,
					COMMAND_SHOW_LOGS
				);
		...
		describe("With gradle tasks", () => {
			...
			describe("Expanded tree", () => {
				...
				it("should build project items at top level", () => {
					assert.ok(
						gradleProjects.length > 0,
						"No gradle projects found"
					);
					assert.strictEqual(
					...
					assert.ok(
						projectItem.parentTreeItem instanceof
							RootProjectTreeItem,
			...
			describe("Task state", () => {
				...
				it("should show a cancelling state", async () => {
					...
					await new CancelBuildCommand(client).run(
						cancellationKey,
						task
					);
					assert.ok(
		...
		describe("With pinned tasks", () => {
			...
			it("should build a pinned task treeitem", async () => {
				...
				assert.strictEqual(
					pinnedTaskItem.label,
					mockTaskDefinition1ForFolder1.script
				);
				assert.strictEqual(
					pinnedTaskItem.tooltip,
					mockTaskDefinition1ForFolder1.description
				);
				assert.strictEqual(
					pinnedTaskItem.task.definition.id,
					mockTaskDefinition1ForFolder1.id
				);
			...
			it("should build a pinned task treeitem with args", async () => {
				...
				await new PinTaskWithArgsCommand(
					pinnedTasksStore,
					gradleTasksTreeDataProvider
				).run(taskItem);
				const projectTreeItems =
				...
				assert.strictEqual(
					pinnedTaskItem.label,
					mockTaskDefinition1ForFolder1.script + " --info"
				);
				assert.strictEqual(
					pinnedTaskItem.tooltip,
					mockTaskDefinition1ForFolder1.description
			...
			it("should build a pinned task treeitem with args when pinning an existing pinned task", async () => {
				...
				assert.ok(
					childrenBefore[0] instanceof PinnedTasksTreeItem,
					"First item is not a PinnedTasksTreeItem"
				);
				const pinnedTasksItem =
					childrenBefore[0] as PinnedTasksTreeItem;
				const pinnedTaskItems = pinnedTasksItem.getChildren();
				assert.strictEqual(pinnedTaskItems?.length, 1);
				const pinnedTaskItemBefore =
					pinnedTaskItems[0] as GradleTaskTreeItem;
				sinon.stub(vscode.window, "showInputBox").resolves("--info "); // intentional trailing space
				await new PinTaskWithArgsCommand(
					pinnedTasksStore,
					gradleTasksTreeDataProvider
				).run(pinnedTaskItemBefore as GradleTaskTreeItem);
				const childrenAfter =
					await gradleTasksTreeDataProvider.getChildren();
				assert.strictEqual(childrenAfter.length, 2);
				assert.ok(
					childrenAfter[0] instanceof PinnedTasksTreeItem,
					"First item is not a PinnedTasksTreeItem"
				);
				const projectTaskTreeItemAfter =
					childrenAfter[0] as PinnedTasksTreeItem;
				const pinnedTaskTreeItems =
					projectTaskTreeItemAfter.getChildren();
				assert.strictEqual(pinnedTaskTreeItems?.length, 2);
				assert.strictEqual(
					(pinnedTaskTreeItems[0] as GradleTaskTreeItem).task
						.definition.script,
					(pinnedTaskTreeItems[1] as GradleTaskTreeItem).task
						.definition.script
				);
				const pinnedTaskWithArgsTreeItem =
					pinnedTaskTreeItems[1] as GradleTaskTreeItem;
				assert.strictEqual(
			...
			it("should unpin a task", async () => {
				...
				const pinnedTaskItemBefore =
					pinnedTaskItems[0] as GradleTaskTreeItem;
				assert.ok(
					pinnedTaskItemBefore instanceof GradleTaskTreeItem,
					"Pinned task is not PinnedTaskTreeItem"
				);
				await new UnpinTaskCommand(
					pinnedTasksStore,
					gradleTasksTreeDataProvider
				).run(pinnedTaskItemBefore);
				const childrenAfter =
					await gradleTasksTreeDataProvider.getChildren();
				assert.strictEqual(childrenAfter.length, 1);
				assert.ok(
					childrenAfter[0] instanceof ProjectTreeItem,
					"Gradle project is not a ProjectTreeItem"
				);
				const projectTreeItemAfter =
					childrenAfter[0] as ProjectTreeItem;
				const projectTreeItemChildrenAfter =
					await gradleTasksTreeDataProvider.getChildrenForProjectTreeItem(
						projectTreeItemAfter
					);
				assert.strictEqual(projectTreeItemChildrenAfter.length, 2);
				assert.ok(
					projectTreeItemChildrenAfter[0] instanceof
						ProjectTaskTreeItem,
					"project item's first children is not a ProjectTaskTreeItem"
				);
				const projectTaskTreeItemAfter =
					projectTreeItemChildrenAfter[0] as ProjectTaskTreeItem;
				const taskTreeItemChildrenAfter =
					projectTaskTreeItemAfter.getChildren();
				assert.strictEqual(
					taskTreeItemChildrenAfter?.length,
					1,
					"no pinned task item should exist"
				);
			});
			...
			it("should unpin all tasks", async () => {
				...
				await new UnpinAllTasksCommand(
					pinnedTasksStore,
					gradleTasksTreeDataProvider
				).run();
				assert.ok(
		...
		describe("With gradle tasks", () => {
			...
			it("should build root RootProject items at top level", async () => {
				const rootProjectItems =
					await gradleTasksTreeDataProvider.getChildren();
				assert.ok(
					rootProjectItems.length > 0,
					"No root gradle projects found"
				);
				assert.strictEqual(
				...
				const rootProjectItem =
					rootProjectItems[0] as RootProjectTreeItem;
				assert.ok(
				...
				assert.strictEqual(
					rootProjectItem.label,
					mockWorkspaceFolder1.name
				);
				assert.strictEqual(
					rootProjectItem.iconPath,
		...
		describe("With pinned tasks", () => {
			...
			it("should build a pinned task treeitem", async () => {
				const workspaceTreeItems =
					await gradleTasksTreeDataProvider.getChildren();
				assert.ok(
					workspaceTreeItems.length > 0,
					"No gradle projects found"
				);
				const workspaceTreeItem =
					workspaceTreeItems[0] as RootProjectTreeItem;
				assert.ok(
				...
				const pinnedTaskTreeItem =
					pinnedTaskTreeItems[0] as GradleTaskTreeItem;
				assert.strictEqual(
