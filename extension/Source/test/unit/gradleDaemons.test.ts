...
describe(getSuiteName("Gradle daemons"), () => {
	...
	it("should refresh the daemons list", async () => {
		...
		await new RefreshDaemonStatusCommand(
			gradleDaemonsTreeDataProvider
		).run();
		assert.ok(
			onDidChangeSpy.calledWith(),
			"onDidChangeTreeData not called"
		);
