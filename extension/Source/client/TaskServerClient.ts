...
import { logger, Logger, LoggerStream, LogVerbosity } from "../logger";
...
export class TaskServerClient implements vscode.Disposable {
	...
	public handleClientReady = async (
		err: Error | undefined,
	): Promise<void> => {
		if (err) {
	...
	public async getBuild(
		...
		return vscode.window.withProgress(
			...
			async (
				...
				const stdOutLoggerStream = new LoggerStream(
					logger,
					LogVerbosity.INFO,
				);
				...
				try {
					return await new Promise((resolve, reject) => {
						...
						getBuildStream
							.on(
								"data",
								async (getBuildReply: GetBuildReply) => {
	...
	public async cancelBuilds(): Promise<void> {
		...
		try {
			...
			logger.error(
				"Error cancelling builds:",
				err.details || err.message,
			);
		}
	}

	public async getNormalizedPackageName(
		name: string,
		...
		try {
			return await new Promise((resolve, reject) => {
				this.grpcClient!.executeCommand(
					...
					(
						err: grpc.ServiceError | null,
						executeCommandReply: ExecuteCommandReply | undefined,
					) => {
						if (err) {
							reject(err);
						} else {
