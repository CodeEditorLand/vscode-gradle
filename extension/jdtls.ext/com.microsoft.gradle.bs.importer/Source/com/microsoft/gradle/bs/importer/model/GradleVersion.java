 ...
 * <p>
 * The implementation is a slimmed down version of the
 * {@code org.gradle.util.GradleVersion}
...
public final class GradleVersion implements Comparable<GradleVersion> {
	...
	private static final Pattern VERSION_PATTERN = Pattern
			.compile("((\\d+)(\\.\\d+)+)(-(\\p{Alpha}+)-(\\d+[a-z]?))?(-(SNAPSHOT|\\d{14}([-+]\\d{4})?))?");
	...
	private GradleVersion(String version) {
		...
		if (!matcher.matches()) {
			throw new IllegalArgumentException(
					String.format("'%s' is not a valid Gradle version string (examples: '1.0', '1.0-rc-1')", version));
		}
