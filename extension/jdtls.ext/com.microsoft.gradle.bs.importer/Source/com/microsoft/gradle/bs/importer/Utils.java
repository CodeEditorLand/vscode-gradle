...
public class Utils {
	...
	public static Map<URI, List<BuildTarget>> getBuildTargetsMappedByProjectPath(
			BuildServerConnection serverConnection) {
		...
		return buildTargets.stream()
				.collect(Collectors.groupingBy(target -> getUriWithoutQuery(target.getId().getUri())));
	}
	...
	public static List<BuildTarget> getBuildTargetsByProjectUri(BuildServerConnection serverConnection,
			URI projectUri) {
		if (projectUri == null) {
		...
		return buildTargets.stream()
				.filter(target -> URIUtil.sameURI(projectUri, getUriWithoutQuery(target.getId().getUri())))
	...
	/**
	 * Add nature to the project if the input nature does not exist in project
	 * description.
	 * 
	...
	public static void addNature(IProject project, String natureId, IProgressMonitor monitor) throws CoreException {
		...
		// abort if the project already has the nature applied or the nature is not
		// defined
	...
	/**
	 * Add the builders to the project description if the input builders do not
	...
	public static void addBuildSpec(IProjectDescription description, ICommand[] buildSpecs) {
		...
		for (ICommand buildSpec : buildSpecs) {
			if (currentBuildSpecs.stream()
					.anyMatch(spec -> Objects.equals(spec.getBuilderName(), buildSpec.getBuilderName()))) {
				continue;
			}
