...
public class EclipseVmUtil {
	...
	/**
	 * Finds a Java VM in the Eclipse VM registry. It will first find the expected
	 * version,
	 * If it is not available, Then it will search the highest version among
	 * [{@code lowestVersion}, {@code highestVersion}].
	 *
	 * If no valid JDK can be found, registers a new one specified by
	 * {@code location}, which is
	...
	private static IVMInstall getCompatibleVMWithHighestVersion(String expectedVersion, String lowestVersion,
		...
		for (IVMInstall compatibleVm : compatibleVMs) {
			...
			if (installLocation == null || !installLocation.exists()
					|| isEmbeddedJre(installLocation.getAbsolutePath())) {
				continue;
			}
	 ...
	 * 'JavaSE-1.6' for version '1.6'.
	 *
	 * @param version the Java version
	 * @return the execution environment or {@link Optional#empty()} if none was
	 *         found
	 */
	public static Optional<IExecutionEnvironment> findExecutionEnvironment(String version) {
		...
		for (IExecutionEnvironment executionEnvironment : JavaRuntime.getExecutionEnvironmentsManager()
	...
	/**
	 * Get all the available JDK installations in the Eclipse VM registry. If
	 * multiple installations
	 * are found for the same major version, the first found one is return.
	 *
	 * The results are returned as map, where key is the major version and value is
	 * the uri string of
	 * the installation path.
	 * <p>
	 * Note: The embedded JRE is excluded.
	 */
	...
	private static String getExecutionEnvironmentId(String version) {
		// the result values correspond to the standard execution environment
		// definitions in the
