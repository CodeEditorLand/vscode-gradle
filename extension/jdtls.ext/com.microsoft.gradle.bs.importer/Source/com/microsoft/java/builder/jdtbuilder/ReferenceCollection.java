...
public class ReferenceCollection {
	...
	// contains no simple names as in just 'a' which is kept in simpleNameReferences
	// instead
	// TODO after #addDependencies, it will contain simple names, though. See
	// ReferenceCollectionTest
	 ...
	 * Subsequent queries of {@link #includes(char[][][], char[][], char[][])} will
	 * report true
	...
	public void addDependencies(String[] typeNameDependencies) {
		// if each qualified type name is already known then all of its subNames can be
		// skipped
		// and its expected that very few qualified names in typeNameDependencies need
		// to be added
		// but could always take 'p1.p2.p3.X' and make all qualified names 'p1' 'p1.p2'
		// 'p1.p2.p3' 'p1.p2.p3.X', then intern
	...
	public boolean includes(char[] simpleName) {
		boolean result = sortedArrayContains(this.simpleNameReferences, simpleName,
				SortedCharArrays.CHAR_ARR_COMPARATOR);
	...
	private boolean doIncludes(char[][][] qualifiedNames, char[][] simpleNames, char[][] rootNames) {
		...
		// if either collection of names is null, it means it contained a well known
		// name so we know it already has a match
	...
	public boolean insideRoot(char[] rootName) {
		...
		if (REFERENCE_COLLECTION_DEBUG) {
			if (result != debugIncludes(rootName)) {
				String message = "Mismatch: " + String.valueOf(rootName) + (result ? " should not " : " should ") //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
	...
	private static <T> boolean intersects(T[] firstSortedArr, T[] secondSortedArr, Comparator<? super T> comparator) {
		...
		for (int i = 0, l = firstSortedArr.length, j = 0, k = secondSortedArr.length; i < l && j < k;) {
			...
			if (compare == 0) {
				 ...
				 * attempt a binary search for the second element to possibly skip a few
				 * elements.
				 */
				i++;
	...
	// the MethodVerifier requests 3 well known types which end up in the reference
	// collection
	// having WellKnownQualifiedNames & WellKnownSimpleNames, saves every type 40
	// bytes
	...
	// each array contains qualified char[][], one for size 2, 3, 4, 5, 6, 7 & the
	// rest
	...
	// TODO: remove once ReferenceCollection.internQualifiedNames(StringSet) is
	// adapted to use java.util.Set, so that git history is preserved
	...
	// TODO: remove once PDE API Tools has been adapted to also use java.util.Set,
	...
	/**
	 * <strong>Note</strong>: this method may change order of the result data, the
	 * new array is always sorted.
	 */
	...
	/**
	 * Use a flyweight cache for the char arrays to avoid duplicated arrays with the
	 * same contents.
	 * After calling this method, identity comparison on the array contents of the
	 * resulting array
	 ...
	 * new array is always sorted.
	 * <p>
	 * Optionally drops very common qualified names from the array to spare some
	 * bytes.
	 *
	...
	// TODO: remove once ReferenceCollection.internSimpleNames(StringSet, boolean)
	// is adapted to use java.util.Set, so that git history is preserved
	...
	// TODO: adjust to use java.util.Set once PDE API Tools have been adapted to use
	// the set version, so that git history is preserved
	...
	private void assertIncludes(boolean expectation, char[][][] qualifiedNames, char[][] simpleNames,
		...
		if (expectation != debugIncludes(qualifiedNames, simpleNames, rootNames)) {
			String message = String.format(
					"Mismatched includes(..): ReferenceCollection([%s], %s, %s).includes([%s], %s, %s)", //$NON-NLS-1$
					qualifiedNamesToString(this.qualifiedNameReferences),
			...
			for (int i = 0; i < sLength; i++) {
				if (debugIncludes(simpleNames[i])) {
					for (int j = 0; j < qLength; j++) {
						...
						if (qualifiedName.length == 1 ? debugIncludes(qualifiedName[0])
