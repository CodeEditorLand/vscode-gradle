 ...
 *
 * Copied from https://github.com/eclipse-jdt/eclipse.jdt.core/blob/master/org.eclipse.jdt.core/model/org/eclipse/jdt/internal/core/builder/IncrementalImageBuilder.java
 *
 * Contributors:
...
public class IncrementalImageBuilder extends AbstractImageBuilder {
	...
	protected IncrementalImageBuilder(JavaBuilder javaBuilder, State buildState, CompilationGroup compilationGroup) {
		...
		this.makeOutputFolderConsistent = JavaCore.ENABLED.equals(
				javaBuilder.javaProject
						.getOption(JavaCore.CORE_JAVA_BUILD_RECREATE_MODIFIED_CLASS_FILES_IN_OUTPUT_FOLDER, true));
	...
	public boolean build(SimpleLookupTable deltas) {
		...
		try {
			...
			if (this.javaBuilder.hasBuildpathErrors()) {
				// if a missing class file was detected in the last build, a build state was
				// saved since its no longer fatal
				// but we need to rebuild every source file since problems were not recorded
				// AND to avoid the infinite build scenario if this project is involved in a
				// cycle, see bug 160550
				...
				findAffectedSourceFilesFromRequiredProjects();
	...
	protected void buildAfterBatchBuild() {
		// called from a batch builder once all source files have been compiled AND some
		// changes
	...
	protected void addAffectedSourceFiles() {
		if (this.qualifiedStrings.size() == 0 && this.simpleStrings.size() == 0)
			return;
		if (this.testImageBuilder != null) {
			this.testImageBuilder.addAffectedSourceFiles(this.qualifiedStrings, this.simpleStrings, this.rootStrings,
					null);
		}
		addAffectedSourceFiles(this.qualifiedStrings, this.simpleStrings, this.rootStrings, null);
	}

	protected void addAffectedSourceFiles(Set<String> qualifiedSet, Set<String> simpleSet, Set<String> rootSet,
	...
	protected void addDependentsOf(IPath path, boolean isStructuralChange, Set<String> qualifiedNames,
			Set<String> simpleNames, Set<String> rootNames) {
		...
		if (isStructuralChange) {
			...
			if (last.length() == TypeConstants.PACKAGE_INFO_NAME.length)
				if (CharOperation.equals(last.toCharArray(), TypeConstants.PACKAGE_INFO_NAME)) {
					...
					/*
					 * https://bugs.eclipse.org/bugs/show_bug.cgi?id=323785, in the case of default
					 * package,
					 * there is no need to blame the package itself as there can be no annotations
					 * or documentation
					 * comment tags in the package-info file that can influence the rest of the
					 * package. Just bail out
	...
	protected boolean checkForClassFileChanges(IResourceDelta binaryDelta, ClasspathMultiDirectory md, int segmentCount)
		...
		// remember that if inclusion & exclusion patterns change then a full build is
		// done
	...
	protected void compile(SourceFile[] units, SourceFile[] additionalUnits, boolean compilingFirstGroup) {
		if (compilingFirstGroup && additionalUnits != null) {
			// add any source file from additionalUnits to units if it defines secondary
			// types
			// otherwise its possible during testing with MAX_AT_ONCE == 1 that a secondary
			// type
			// can cause an infinite loop as it alternates between not found and defined,
			// see bug 146324
	...
	protected void deleteGeneratedFiles(IFile[] deletedGeneratedFiles) {
		...
		try {
			for (int j = deletedGeneratedFiles.length; --j >= 0;) {
				...
				this.previousSourceFiles = null; // existing source files did not see it as deleted since they were
													// compiled before it was
				...
				if (p != null) {
					...
					if (binaryDelta != null) {
						if (bLocation instanceof ClasspathJar) {
							...
							return false; // do full build since jar file was changed (added/removed were caught as
											// classpath change)
						}
						if (binaryDelta.getKind() == IResourceDelta.ADDED
							...
							return false; // added/removed binary folder should not make it here (classpath change), but
											// handle anyways
						}
			...
			case IResource.FOLDER:
				switch (binaryDelta.getKind()) {
					...
					case IResourceDelta.REMOVED:
						...
						if (binaryDelta.getKind() == IResourceDelta.ADDED) {
							// see if any known source file is from the same package... classpath already
							// includes new package
	...
	protected boolean findSourceFiles(IResourceDelta delta) throws CoreException {
		...
		for (int i = 0, l = this.sourceLocations.length; i < l; i++) {
			...
			if (this.makeOutputFolderConsistent && md.hasIndependentOutputFolder
					&& !visited.contains(md.binaryFolder)) {
				// even a project which acts as its own source folder can have an
				// independent/nested output folder
			...
			if (md.sourceFolder.equals(this.javaBuilder.currentProject)) {
				...
				if (sourceDelta != null) {
					if (sourceDelta.getKind() == IResourceDelta.REMOVED) {
						...
						return false; // removed source folder should not make it here, but handle anyways (ADDED is
										// supported)
					}
				...
				switch (sourceDelta.getKind()) {
					case IResourceDelta.ADDED:
						if (!isExcluded) {
							...
							createFolder(addedPackagePath, md.binaryFolder); // ensure package exists in the output
																				// folder
							// see if any known source file is from the same package... classpath already
							// includes new package
					...
					case IResourceDelta.REMOVED:
						if (isExcluded) {
							// since this folder is excluded then there is nothing to delete (from this md),
							// but must walk any included subfolders
							...
							for (int i = 0, l = this.sourceLocations.length; i < l; i++) {
								if (this.sourceLocations[i].sourceFolder.getFolder(removedPackagePath).exists()) {
									...
									if (md.hasIndependentOutputFolder)
										createFolder(removedPackagePath, md.binaryFolder); // ensure package exists in
																							// the output folder
						...
						if ((sourceDelta.getFlags() & IResourceDelta.MOVED_TO) != 0) {
							...
							// see bug 163200
							IResource movedFolder = this.javaBuilder.workspaceRoot
									.getFolder(sourceDelta.getMovedToPath());
						...
						// add dependents even when the package thinks it does not exist to be on the
						// safe side
						if (JavaBuilder.DEBUG)
				...
				if (org.eclipse.jdt.internal.core.util.Util.isJavaLikeFileName(resourceName)) {
					...
					switch (sourceDelta.getKind()) {
						case IResourceDelta.ADDED:
							...
							if (!this.newState.isDuplicateLocator(typeName, typeLocator)) { // adding dependents results
																							// in 2 duplicate errors
								if (JavaBuilder.DEBUG)
						...
						case IResourceDelta.REMOVED:
							...
							if (definedTypeNames == null) { // defined a single type matching typePath
								...
								addDependentsOf(typePath, true); // add dependents of the source file since it may be
																	// involved in a name collision
								if (definedTypeNames.length > 0) { // skip it if it failed to successfully define a type
									IPath packagePath = typePath.removeLastSegments(1);
					...
					switch (sourceDelta.getKind()) {
						case IResourceDelta.ADDED:
							...
							createFolder(resourcePath.removeLastSegments(1), md.binaryFolder); // ensure package exists
	...
	protected void removeSecondaryTypes() throws CoreException {
		if (this.secondaryTypesToRemove != null) { // delayed deleting secondary types until the end of the compile loop
			...
			if (this.previousSourceFiles != null)
				this.previousSourceFiles = null; // cannot optimize recompile case when a secondary type is deleted, see
													// 181269
		}
	}
	...
	protected void updateProblemsFor(SourceFile sourceFile, CompilationResult result) throws CoreException {
		if (CharOperation.equals(sourceFile.getMainTypeName(), TypeConstants.PACKAGE_INFO_NAME)) {
			...
			if (findMarkers.length > 0) {
				...
				// trigger a full build, so marker is cleared also from packages in other source
				// folders
				throw new AbortCompilation(true,
						new AbortIncrementalBuildException(new String(TypeConstants.PACKAGE_INFO_NAME)));
			}
		}
			...
			try {
				...
				if (e.getStatus().getCode() == IResourceStatus.CASE_VARIANT_EXISTS) {
					...
					if (status instanceof IResourceStatus) {
						...
						if (fromSameFile) {
							// file is defined by the same compilationUnit, but won't be deleted until later
							// so do it now
	...
	protected boolean writeClassFileCheck(IFile file, String fileName, byte[] newBytes) throws CoreException {
		try {
			...
			if (e instanceof CoreException) {
				// assuming a ResourceException during IFile.getContents(), treat it like a
				// corrupt file
				addDependentsOf(new Path(fileName), true);
				this.newState.wasStructurallyChanged(fileName);
			} else {
