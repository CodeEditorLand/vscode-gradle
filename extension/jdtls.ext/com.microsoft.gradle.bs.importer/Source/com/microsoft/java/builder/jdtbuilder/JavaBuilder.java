 ...
 *
 * Copied from https://github.com/eclipse-jdt/eclipse.jdt.core/blob/master/org.eclipse.jdt.core/model/org/eclipse/jdt/internal/core/builder/JavaBuilder.java
 *
 * Contributors:
...
public class JavaBuilder extends IncrementalProjectBuilder {
	...
	SimpleLookupTable binaryLocationsPerProject; // maps a project to its binary resources (output folders, class
													// folders, zip/jar files)
	...
	/**
	 * Bug 549457: In case auto-building on a JDT core settings change (e.g.
	 * compiler compliance) is not desired,
	 * specify VM property:
	 * {@code -Dorg.eclipse.disableAutoBuildOnSettingsChange=true}
	 */
	private static final boolean DISABLE_AUTO_BUILDING_ON_SETTINGS_CHANGE = Boolean
			.getBoolean("org.eclipse.disableAutoBuildOnSettingsChange"); //$NON-NLS-1$
	private static final IPath JDT_CORE_SETTINGS_PATH = Path.fromPortableString(
			JavaProject.DEFAULT_PREFERENCES_DIRNAME + IPath.SEPARATOR + JavaProject.JAVA_CORE_PREFS_FILE);

	/**
	 * A list of project names that have been built.
	 * This list is used to reset the JavaModel.existingExternalFiles cache when a
	 * build cycle begins
	...
	public static IMarker[] getProblemsFor(IResource resource) {
		try {
			if (resource != null && resource.exists()) {
				IMarker[] markers = resource.findMarkers(IJavaModelMarker.JAVA_MODEL_PROBLEM_MARKER, false,
	...
	protected IProject[] build(int kind, Map ignored, IProgressMonitor monitor) throws CoreException {
		org.eclipse.jdt.ls.core.internal.preferences.Preferences preferences = JavaLanguageServerPlugin
				.getPreferencesManager().getPreferences();
		...
		try {
			...
			if (isWorthBuilding()) {
				if (kind == FULL_BUILD) {
					...
					if ((this.lastState = getLastState(this.currentProject)) == null) {
						if (DEBUG)
							System.out
									.println("JavaBuilder: Performing full build since last saved state was not found"); //$NON-NLS-1$
						buildAll();
					} else if (hasClasspathChanged()) {
						// if the output location changes, do not delete the binary files from old
						// location
						...
						buildAll();
							...
							|| this.testNameEnvironment.sourceLocations.length > 0) {
						...
						if (deltas == null) {
							...
							if (hasJdtCoreSettingsChange(deltas) && !DISABLE_AUTO_BUILDING_ON_SETTINGS_CHANGE) {
								if (DEBUG)
									System.out.println(
											"JavaBuilder: Performing full build since project settings have changed"); //$NON-NLS-1$
								buildAll();
							} else {
								buildDeltas(deltas);
							}
			...
			// do not log this exception since its thrown to handle aborted compiles because
			...
			if (!ok)
				// If the build failed, clear the previously built state, forcing a full build
				// next time.
				clearLastState();
			this.notifier.done();
			cleanup();
	...
	private void buildAll() {
		...
		clearLastState();
		BatchImageBuilder imageBuilder = createBatchImageBuilder(this, true, CompilationGroup.MAIN);
		BatchImageBuilder testImageBuilder = createBatchImageBuilder(imageBuilder, true, CompilationGroup.TEST);
		imageBuilder.build();
		if (testImageBuilder.sourceLocations.length > 0) {
			// Note: testImageBuilder *MUST* have a separate output folder, or it will
			// delete the files created by imageBuilder.build()
	...
	private void buildDeltas(SimpleLookupTable deltas) {
		...
		clearLastState(); // clear the previously built state so if the build fails, a full build will
							// occur next time
		IncrementalImageBuilder imageBuilder = createIncrementalImageBuilder(this);
	...
	private void clearLastState() {
		BuildStateManager.getBuildStateManager().setLastBuiltState(this.currentProject, null);
	}
				...
				if (!this.lastState.wasStructurallyChanged(p, s)) { // see if we can skip its delta
					...
					for (int j = 0, m = classFoldersAndJars.length; j < m; j++) {
						if (classFoldersAndJars[j].isOutputFolder())
							classFoldersAndJars[j] = null; // can ignore output folder since project was not
	...
	public State getLastState(IProject project) {
		return (State) BuildStateManager.getBuildStateManager().getLastBuiltState(project, this.notifier.monitor);
	}

	/*
	 * Return the list of projects for which it requires a resource delta. This
	 * builder's project
	 * is implicitly included and need not be specified. Builders must re-specify
	 * the list
	 * of interesting projects every time they are run as this is not carried
	 * forward
	 * beyond the next build. Missing projects should be specified but will be
	 * ignored until
	...
	private boolean hasClasspathChanged(CompilationGroup compilationGroup) {
		ClasspathMultiDirectory[] newSourceLocations = (compilationGroup == CompilationGroup.MAIN ? this.nameEnvironment
				: this.testNameEnvironment).sourceLocations;
		ClasspathMultiDirectory[] oldSourceLocations = compilationGroup == CompilationGroup.MAIN
				? this.lastState.sourceLocations
				: this.lastState.testSourceLocations;
		...
		ClasspathLocation[] newBinaryLocations = (compilationGroup == CompilationGroup.MAIN ? this.nameEnvironment
				: this.testNameEnvironment).binaryLocations;
		ClasspathLocation[] oldBinaryLocations = compilationGroup == CompilationGroup.MAIN
				? this.lastState.binaryLocations
				: this.lastState.testBinaryLocations;
		...
		for (n = o = 0; n < newLength && o < oldLength; n++, o++) {
			if (newBinaryLocations[n].equals(oldBinaryLocations[o]))
				continue;
			if (DEBUG) {
				System.out.println("JavaBuilder: New test location: " + newBinaryLocations[n] //$NON-NLS-1$
						+ "\n!= old test location: " + oldBinaryLocations[o]); //$NON-NLS-1$
	...
	private boolean hasStructuralDelta() {
		...
		if (delta != null && delta.getKind() != IResourceDelta.NO_CHANGE) {
			ClasspathLocation[] classFoldersAndJars = (ClasspathLocation[]) this.binaryLocationsPerProject
					.get(this.currentProject);
			if (classFoldersAndJars != null) {
				for (int i = 0, l = classFoldersAndJars.length; i < l; i++) {
					ClasspathLocation classFolderOrJar = classFoldersAndJars[i]; // either a .class file folder or a
																					// zip/jar file
	...
	private int initializeBuilder(int kind, boolean forBuild) throws CoreException {
		...
		if (forBuild) {
			...
			// Flush the existing external files cache if this is the beginning of a build
			// cycle
		...
		this.nameEnvironment = createNameEnvironment(this.workspaceRoot, this.javaProject,
				this.binaryLocationsPerProject, this.notifier, CompilationGroup.MAIN);
		this.testNameEnvironment = createNameEnvironment(this.workspaceRoot, this.javaProject,
				this.binaryLocationsPerProject, this.notifier, CompilationGroup.TEST);
	...
	private boolean isClasspathBroken(JavaProject jProj, boolean tryRepair) throws CoreException {
		IMarker[] markers = jProj.getProject().findMarkers(IJavaModelMarker.BUILDPATH_PROBLEM_MARKER, false,
				IResource.DEPTH_ZERO);
		for (int i = 0, l = markers.length; i < l; i++) {
			if (markers[i].getAttribute(IMarker.SEVERITY, -1) == IMarker.SEVERITY_ERROR) {
				if (tryRepair) {
					...
					if (code instanceof Integer
							&& ((Integer) code) == IJavaModelStatusConstants.CP_INVALID_EXTERNAL_ANNOTATION_PATH) {
	...
	private boolean isWorthBuilding() throws CoreException {
		...
		// make sure all prereq projects have valid build states... only when aborting
		// builds since projects in cycles do not have build states
		// except for projects involved in a 'warning' cycle (see below)
		IProject[] requiredProjects = getRequiredProjects(false);
		for (int i = 0, l = requiredProjects.length; i < l; i++) {
			IProject p = requiredProjects[i];
			if (getLastState(p) == null) {
				// The prereq project has no build state: if this prereq project has a 'warning'
				// cycle marker then allow build (see bug id 23357)
	...
	void mustPropagateStructuralChanges() {
		...
		this.javaProject.updateCycleParticipants(new ArrayList(), cycleParticipants, new HashMap<>(),
				this.workspaceRoot, new HashSet(3), null);
		...
		while (i.hasNext()) {
			...
			if (participantPath != currentPath) {
				...
				if (hasBeenBuilt(project)) {
					if (DEBUG)
						System.out.println("JavaBuilder: Requesting another build iteration since cycle participant " //$NON-NLS-1$
								+ project.getName()
	...
	private void recordNewState(State state) {
		...
		// state.dump();
		BuildStateManager.getBuildStateManager().setLastBuiltState(this.currentProject, state);
	}
	...
	protected NameEnvironment createNameEnvironment(IWorkspaceRoot root, JavaProject javaProject,
		...
		return new NameEnvironment(root, javaProject, binaryLocationsPerProject, notifier, compilationGroup);
	}
}
