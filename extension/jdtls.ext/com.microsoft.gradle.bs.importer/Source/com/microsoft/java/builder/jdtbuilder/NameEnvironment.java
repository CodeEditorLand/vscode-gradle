 ...
 *
 * Copied from https://github.com/eclipse-jdt/eclipse.jdt.core/blob/master/org.eclipse.jdt.core/model/org/eclipse/jdt/internal/core/builder/NameEnvironment.java
 *
 * Contributors:
...
public class NameEnvironment implements IModuleAwareNameEnvironment, SuffixConstants {
	...
	SimpleSet initialTypeNames; // assumed that each name is of the form "a/b/ClassName", or, if a module is
								// given: "my.mod:a/b/ClassName"
	...
	public NameEnvironment(IWorkspaceRoot root, JavaProject javaProject, SimpleLookupTable binaryLocationsPerProject,
			BuildNotifier notifier, CompilationGroup compilationGroup) throws CoreException {
	 ...
	 *
	 * 2. zip files:
	 * D:/j9/lib/jclMax/classes.zip[CPE_LIBRARY][K_BINARY][sourcePath:d:/j9/lib/
	 * jclMax/source/source.zip]
	...
	private void computeClasspathLocations(
		...
		IClasspathEntry[] classpathEntries = javaProject
				.getExpandedClasspath(this.compilationGroup == CompilationGroup.MAIN);
			...
			switch (entry.getEntryKind()) {
					...
					nextPrereqEntry: for (int j = 0, m = prereqClasspathEntries.length; j < m; j++) {
						...
						if (prereqEntry.getEntryKind() == IClasspathEntry.CPE_SOURCE) {
							...
							if (srcExtAnnotPath == null) {
								// search in other sources contributing to the same binary location (that other
								// loc will be skipped below, due to seen.contains()):
				...
				case IClasspathEntry.CPE_LIBRARY:
					if (target instanceof IResource) {
						...
						bLocations.add(bLocation);
						// TODO: Ideally we need to do something like mapToModulePathEntry using the
						// path and if it is indeed
						// a module path entry, then add the corresponding entry here, but that would
						// need the target platform
						...
						AccessRuleSet accessRuleSet = (JavaCore.IGNORE
								...
								&& JavaCore.IGNORE.equals(
										javaProject.getOption(JavaCore.COMPILER_PB_DISCOURAGED_REFERENCE, true)))
												? null
												: entry.getAccessRuleSet();
						String release = JavaCore.ENABLED
								.equals(javaProject.getOption(JavaCore.COMPILER_RELEASE, false)) ? compliance : null;
						ClasspathLocation bLocation = null;
		...
		if (JavaCore.ENABLED.equals(
				javaProject.getOption(JavaCore.CORE_JAVA_BUILD_EXTERNAL_ANNOTATIONS_FROM_ALL_LOCATIONS, true))) {
		...
		// now split the classpath locations... place the output folders ahead of the
		// other .class file folders & jars
		...
		if (!sLocations.isEmpty()) {
			...
			if (moduleEntries != null && projectModule != null) {
				try {
					...
					if (!moduleEntries.containsKey(sourceModule.getElementName())) { // can be registered already, if
																						// patching
			...
			next: for (int i = 0, l = this.sourceLocations.length; i < l; i++) {
				...
				for (int j = 0; j < i; j++) { // compare against previously walked source folders
					...
					if (outputPath.equals(previousMd.binaryFolder.getFullPath())) {
						if ((eeaPath == null) != (previousMd.externalAnnotationPath == null) // one has eeaPath other
																								// doesn't ?
								&& md.isOnModulePath == previousMd.isOnModulePath
								&& md.accessRuleSet == previousMd.accessRuleSet) {
							// only relevant difference between md and previousMd is a non-conflicting
							// eea-path, so unify into one entry *with* eea-path:
							if (eeaPath == null)
								eeaPath = previousMd.externalAnnotationPath;
							int prev = outputFolders.indexOf(previousMd);
							if (prev != -1) {
								outputFolders.set(prev, new ClasspathDirectory(md.binaryFolder, true, md.accessRuleSet,
										new Path(eeaPath), md.isOnModulePath));
								continue next;
							}
						}
				...
				// also tag each source folder whose output folder is an independent folder & is
				// not also a source folder
		...
		// combine the output folders with the binary folders & jars... place the output
		// folders before other .class file folders & jars
	...
	/**
	 * Returns the patched module if that is served by the current (binary)
	 * location.
	 */
	IModule collectModuleEntries(ClasspathLocation bLocation, IPath path, boolean isOnModulePath,
		...
		if (bLocation instanceof IMultiModuleEntry) {
			...
			if (patchedModuleName != null) {
				...
				if (module != null)
					return module;
				// TODO(SHMOD): report problem: patchedModuleName didn't match a module from
				// this location
			}
			...
			if (module != null) {
				...
				bLocation.limitModuleNames = limitModules;
				bLocation.patchModuleName = patchedModuleName;
				if (limitModules == null || limitModules == ClasspathJrt.NO_LIMIT_MODULES
						|| limitModules.contains(moduleName)) {
					moduleEntries.put(moduleName, binaryModulePathEntry);
					if (patchedModuleName != null) {
						if (moduleName.equals(patchedModuleName))
							return module;
	...
	public void cleanup() {
		...
		// assume modulePathEntries are cleaned-up via the corresponding
		// source/binaryLocations
	}
	...
	private NameEnvironmentAnswer findClass(String qualifiedTypeName, char[] typeName, LookupStrategy strategy,
		...
		if (this.initialTypeNames != null && this.initialTypeNames.includes(moduleQualifiedName)) {
			if (this.isIncrementalBuild)
				// catch the case that a type inside a source file has been renamed but other
				// class files are looking for it
				throw new AbortCompilation(true, new AbortIncrementalBuildException(qualifiedTypeName));
			return null; // looking for a file which we know was provided at the beginning of the
							// compilation
		}

		if (this.additionalUnits != null && this.sourceLocations.length > 0) {
			// if an additional source file is waiting to be compiled, answer it BUT not if
			// this is a secondary type search
			// if we answer X.java & it no longer defines Y then the binary type looking for
			// Y will think the class path is wrong
			// let the recompile loop fix up dependents when the secondary type Y has been
			// deleted from X.java
			// Only enclosing type names are present in the additional units table, so strip
			// off inner class specifications
			// when doing the lookup (https://bugs.eclipse.org/372418).
			// Also take care of $ in the name of the class
			// (https://bugs.eclipse.org/377401)
			// and prefer name with '$' if unit exists rather than failing to search for
			// nested class (https://bugs.eclipse.org/392727)
	...
	public char[][] getAllAutomaticModules() {
		if (this.modulePathEntries == null)
			return CharOperation.NO_CHAR_CHAR;
		Set<char[]> set = this.modulePathEntries.values().stream().filter(m -> m.isAutomaticModule())
