 ...
 *
 * Copied from https://github.com/eclipse-jdt/eclipse.jdt.core/blob/master/org.eclipse.jdt.core/model/org/eclipse/jdt/internal/core/builder/BatchImageBuilder.java
 *
 * Contributors:
...
public class BatchImageBuilder extends AbstractImageBuilder {
	...
	IncrementalImageBuilder incrementalBuilder; // if annotations or secondary types have to be processed after the
												// compile loop
	ArrayList secondaryTypes; // qualified names for all secondary types found during batch compile
	Set<String> typeLocatorsWithUndefinedTypes; // type locators for all source files with errors that may be caused by
												// 'not found' secondary types
	...
	protected BatchImageBuilder(BatchImageBuilder batchImageBuilder, boolean buildStarting,
	...
	public void build() {
		...
		try {
			this.notifier
					.subTask(Messages.bind(Messages.build_cleaningOutput, this.javaBuilder.currentProject.getName()));
	...
	protected void cleanOutputFolders(boolean copyBack) throws CoreException {
		...
		if (deleteAll) {
			if (this.compilationGroup != CompilationGroup.TEST) {
				// CompilationGroup.MAIN is done first, so this notifies the participants only
				// once
				// calling this for CompilationGroup.TEST could cases generated files for
				// CompilationGroup.MAIN to be deleted.
			...
			for (int i = 0, l = this.sourceLocations.length; i < l; i++) {
				...
				if (sourceLocation.hasIndependentOutputFolder) {
					...
					final char[][] exclusionPatterns = isOutputFolder
							? sourceLocation.exclusionPatterns
							: null; // ignore exclusionPatterns if output folder == another source folder... not
									// this one
					final char[][] inclusionPatterns = isOutputFolder
							? sourceLocation.inclusionPatterns
							: null; // ignore inclusionPatterns if output folder == another source folder... not
									// this one
					sourceLocation.binaryFolder.accept(
							new IResourceProxyVisitor() {
								...
								public boolean visit(IResourceProxy proxy) throws CoreException {
									...
									if (exclusionPatterns != null && inclusionPatterns == null) // must walk children if
																								// inclusionPatterns !=
																								// null
					...
					public boolean visit(IResourceProxy proxy) throws CoreException {
						...
						switch (proxy.getType()) {
							case IResource.FILE:
								...
								createFolder(partialPath.removeLastSegments(1), outputFolder); // ensure package folder
																								// exists
	...
	protected void processAnnotationResults(CompilationParticipantResult[] results) {
		// to compile the compilation participant results, we need to incrementally
		// recompile all affected types
	...
	protected void rebuildTypesAffectedBySecondaryTypes() {
		// to compile types that could not find 'missing' secondary types because of
		// multiple
		// compile groups, we need to incrementally recompile all affected types as if
		// the missing
