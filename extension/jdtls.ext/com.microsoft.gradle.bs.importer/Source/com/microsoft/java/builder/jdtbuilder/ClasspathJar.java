 ...
 *
 * Copied from https://github.com/eclipse-jdt/eclipse.jdt.core/blob/master/org.eclipse.jdt.core/model/org/eclipse/jdt/internal/core/builder/ClasspathJar.java
 *
 * Contributors:
...
public class ClasspathJar extends ClasspathLocation {
		...
		if (this.closeZipFileAtEnd) {
				...
				try {
					...
					if (org.eclipse.jdt.internal.core.JavaModelManager.ZIP_ACCESS_VERBOSE) {
						System.out.println("(" + Thread.currentThread() //$NON-NLS-1$
								+ ") [ClasspathJar.cleanup()] Closed ZipFile on " + this.zipFilename); //$NON-NLS-1$
					}
				} catch (IOException e) { // ignore it
					JavaCore.getPlugin().getLog().log(new Status(IStatus.ERROR, JavaCore.PLUGIN_ID,
							"Error closing " + this.zipFile.getName(), e)); //$NON-NLS-1$
				}
				this.zipFile = null;
			}
			if (this.annotationZipFile != null) {
				try {
					...
					if (org.eclipse.jdt.internal.core.JavaModelManager.ZIP_ACCESS_VERBOSE) {
						System.out.println("(" + Thread.currentThread() //$NON-NLS-1$
								+ ") [ClasspathJar.cleanup()] Closed Annotation ZipFile on " + this.zipFilename); //$NON-NLS-1$
					}
				} catch (IOException e) { // ignore it
					JavaCore.getPlugin().getLog().log(new Status(IStatus.ERROR, JavaCore.PLUGIN_ID,
			...
			if (this.zipFile != null && org.eclipse.jdt.internal.core.JavaModelManager.ZIP_ACCESS_VERBOSE) {
				try {
					this.zipFile.size();
					System.out.println("(" + Thread.currentThread() //$NON-NLS-1$
							+ ") [ClasspathJar.cleanup()] ZipFile NOT closed on " + this.zipFilename); //$NON-NLS-1$
	...
	public boolean hasCompilationUnit(String pkgName, String moduleName) {
		if (scanContent()) {
			...
			// Even if knownPackageNames contained the pkg we're looking for, we still need
			// to verify
			// that the package in this jar actually contains at least one .class file
			// (since
	...
	protected IBinaryType decorateWithExternalAnnotations(IBinaryType reader, String fileNameWithoutExtension) {
		if (scanContent()) { // ensure zipFile is initialized
			String qualifiedBinaryFileName = fileNameWithoutExtension
