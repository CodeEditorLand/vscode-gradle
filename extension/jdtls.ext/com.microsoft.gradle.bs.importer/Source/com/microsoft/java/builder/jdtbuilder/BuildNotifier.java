 ...
 *
 * Copied from https://github.com/eclipse-jdt/eclipse.jdt.core/blob/master/org.eclipse.jdt.core/model/org/eclipse/jdt/internal/core/builder/BuildNotifier.java
 *
 * Contributors:
...
public class BuildNotifier {
	...
	public void checkCancel() {
		if (this.monitor != null && this.monitor.isCanceled()) {
			...
			if (this.buildKind == IncrementalProjectBuilder.AUTO_BUILD) {
				// Since jdt can not resume the incremental build we follow that request only
				// after a period of grace
				// to increase the chance the cancel happens between project builds and not
		...
		if (this.interruptSupplier.getAsBoolean()) {
			...
			if (getBuildDurationInMs() > millisecondsBeforeInterruptAutoBuild) {
				// This is a trade off between UI responsiveness and additional work to restart
				// autobuild.
				// We cancel autobuild to avoid possible UI freeze where the user can not
				// manually cancel.
				throw new OperationCanceledException();
			}
		}
	 ...
	 * of being cancelled. Certain places (those callable indirectly from the
	 * compiler)
	...
	/**
	 * Update the problem counts from one compilation result given the old and new
	 * problems,
