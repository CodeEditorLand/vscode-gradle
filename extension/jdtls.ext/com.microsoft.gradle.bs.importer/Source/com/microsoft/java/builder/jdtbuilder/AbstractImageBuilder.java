 ...
 *
 * Copied from https://github.com/eclipse-jdt/eclipse.jdt.core/blob/master/org.eclipse.jdt.core/model/org/eclipse/jdt/internal/core/builder/AbstractImageBuilder.java
 *
 * Contributors:
...
public abstract class AbstractImageBuilder implements ICompilerRequestor, ICompilationUnitLocator {
	...
	protected AbstractImageBuilder(JavaBuilder javaBuilder, boolean buildStarting, State newState,
		...
		this.nameEnvironment = compilationGroup == CompilationGroup.TEST ? javaBuilder.testNameEnvironment
				: javaBuilder.nameEnvironment;
			...
			if (this.javaBuilder.participants != null) {
				for (int i = 0, l = this.javaBuilder.participants.length; i < l; i++) {
					if (this.javaBuilder.participants[i].isAnnotationProcessor()) {
						// initialize this set so the builder knows to gather CUs that define Annotation
						// types
						// each Annotation processor participant is then asked to process these files
						// AFTER
						// the compile loop. The normal dependency loop will then recompile all affected
						// types
	...
	public void acceptResult(CompilationResult result) {
		...
		// remove the old problems. Plus delete additional class files that no longer
		// exist.
		...
		if (!(resultCU instanceof SourceFile)) {
			...
			// we should never have to report errors etc for those, but this entire
			// construction is a kludge,
		...
		if (!this.workQueue.isCompiled(compilationUnit)) {
			...
			try {
				updateProblemsFor(compilationUnit, result); // record compilation problems before potentially adding
															// duplicate errors
			...
			if (result.hasInconsistentToplevelHierarchies)
				// ensure that this file is always retrieved from source for the rest of the
				// build
			...
			for (int i = 0; i < length; i++) {
				...
				// Look for a possible collision, if one exists, report an error but do not
				...
				if (isNestedType) {
					...
					if (this.newState.isDuplicateLocator(qualifiedTypeName, typeLocator)) {
						...
						if (mainType == null) {
							try {
								mainTypeName = compilationUnit.initialTypeName; // slash separated qualified name
																				// "p1/p1/A"
						...
						if (TypeConstants.MODULE_INFO_NAME_STRING.equals(qualifiedTypeName)) {
							createProblemFor(compilationUnit.resource, type, Messages.build_duplicateModuleInfo,
									JavaCore.ERROR);
						} else {
				...
				for (int j = 0, l = this.javaBuilder.participants == null ? 0
						: this.javaBuilder.participants.length; j < l; j++) {
			...
			if (result.hasAnnotations && this.filesWithAnnotations != null) {
				...
				// AnnotationBinding[] bindings =
				// result.annotations.stream().flatMap(Arrays::stream).filter(Objects::nonNull).toArray(AnnotationBinding[]::new);
	...
	protected void compile(SourceFile[] units) {
		if (this.filesWithAnnotations != null && this.filesWithAnnotations.size() > 0)
			// will add files that have annotations in acceptResult() & then
			// processAnnotations() before exitting this method
		...
		if (this.compiledAllAtOnce) {
			...
			SourceFile[] remainingUnits = new SourceFile[unitsLength]; // copy of units, removing units when about to
																		// compile
	...
	protected void compile(SourceFile[] units, SourceFile[] additionalUnits, boolean compilingFirstGroup) {
		...
		// extend additionalFilenames with all hierarchical problem types found during
		// this entire build
		...
		try {
			...
			// the Compiler failed after the user has chose to cancel... likely due to an
			// OutOfMemory error
		} finally {
		...
		try {
			...
			marker.setAttributes(
					new String[] { IMarker.MESSAGE, IMarker.SEVERITY, IMarker.CHAR_START, IMarker.CHAR_END,
							IMarker.SOURCE_ID },
					new Object[] { message, Integer.valueOf(severity), Integer.valueOf(start), Integer.valueOf(end),
	...
	protected void finishedWith(String sourceLocator, CompilationResult result, char[] mainTypeName,
		...
		if (duplicateTypeNames == null) {
			this.newState.record(sourceLocator, result.qualifiedReferences, result.simpleNameReferences,
					result.rootReferences, mainTypeName, definedTypeNames);
			return;
		}

		char[][] simpleRefs = result.simpleNameReferences;
		// for each duplicate type p1.p2.A, add the type name A (package was already
		// added)
		...
		this.newState.record(sourceLocator, result.qualifiedReferences, simpleRefs, result.rootReferences, mainTypeName,
				definedTypeNames);
	}
	...
	protected boolean isExcludedFromProject(IPath childPath) throws JavaModelException {
		// answer whether the folder should be ignored when walking the project as a
		// source folder
	...
	protected CompilationParticipantResult[] notifyParticipants(SourceFile[] unitsAboutToCompile) {
		...
		// and if so should we pass the generated files from the each processor to the
		// others to process?
		// and what happens if some participants do not expect to be called with only a
		// few files, after seeing 'all' the files?
		for (int i = 0, l = this.javaBuilder.participants.length; i < l; i++)
	...
	protected void processAnnotations(CompilationParticipantResult[] results) {
		...
		// even if no files have annotations, must still tell every annotation processor
		// in case the file used to have them
		for (int i = 0, l = this.javaBuilder.participants.length; i < l; i++)
			if (this.javaBuilder.participants[i].isAnnotationProcessor())
	...
	protected void recordParticipantResult(CompilationParticipantResult result) {
		...
		// must be called after we're finished with the compilation unit results but
		// before incremental loop adds affected files
	 ...
	 * - it's {@link IMarker#SOURCE_ID} attribute is positioned to
	 * {@link JavaBuilder#SOURCE_ID} if
	 * the problem was generated by JDT; else the {@link IMarker#SOURCE_ID}
	 * attribute is
	...
	protected void storeProblemsFor(SourceFile sourceFile, CategorizedProblem[] problems) throws CoreException {
		if (sourceFile == null || problems == null || problems.length == 0)
			return;
		// once a classpath error is found, ignore all other problems for this project
		// so the user can see the main error
		// but still try to compile as many source files as possible to help the case
		// when the base libraries are in source
		...
		problems: for (int i = 0, l = problems.length; i < l; i++) {
			...
			// we may use a different resource for certain problems such as
			// IProblem.MissingNonNullByDefaultAnnotationOnPackage
			// but at the start of the next problem we should reset it to the source file's
			// resource
			...
			switch (id) {
				...
				case IProblem.IsClassPathCorrectWithReferencingType:
					buildPathProblemMessage = Messages.bind(Messages.build_incompleteClassPath,
							problem.getArguments()[0]);
					break;
				case IProblem.UndefinedModuleAddReads:
					buildPathProblemMessage = Messages.bind(Messages.build_errorOnModuleDirective,
							problem.getMessage());
					break;
			}
			if (buildPathProblemMessage != null) {
				...
				// insert extra classpath problem, and make it the only problem for this project
				// (optional)
				if (isInvalidClasspathError && JavaCore.ABORT.equals(
					...
					JavaBuilder.removeProblemsAndTasksFor(this.javaBuilder.currentProject); // make this the only
				...
				// even if we're not keeping more markers, still fall through rest of the
				// problem reporting, so that offending
				// IsClassPathCorrect problem gets recorded since it may help locate the
				// offending reference
			}
			...
			if (IJavaModelMarker.JAVA_MODEL_PROBLEM_MARKER.equals(markerType)
				...
				if (id == IProblem.MissingNonNullByDefaultAnnotationOnPackage
					...
					// for this kind of problem, marker needs to be created on the package instead
					// of on the source file
					// see bug 372012
				...
				attributes.put(JAVA_PROBLEM_MARKER_ATTRIBUTE_NAMES[1],
						problem.isError() ? S_ERROR : problem.isWarning() ? S_WARNING : S_INFO);// SEVERITY
				...
				attributes.put(JAVA_PROBLEM_MARKER_ATTRIBUTE_NAMES[6],
